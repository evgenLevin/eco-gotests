...
func gracefulRestartTestWithBFD(
	...
	By("Aplying the MachineConfig to add nft rule that blocks BGP and BFD port")
	createMCAndWaitforMCPStable(tsparams.BlockBGPBFDPorts)

	By("Checking that BFD sessions are down")
	Expect(netenv.BFDHasStatus(frrPod, ipaddr.RemovePrefix(ipv4NodeAddrList[0]), "init")).
